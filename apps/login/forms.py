...
class RegistrationForm(forms.ModelForm):
    ...
    first_name = forms.CharField(max_length=45, validators=[validateLengthGreaterThanTwo, validateOnlyLetters], label='Name:')
    # last_name = forms.CharField(max_length=45, validators=[validateLengthGreaterThanTwo, validateOnlyLetters])
    email = forms.EmailField(widget=forms.TextInput, label='Email:')
    username = forms.CharField(max_length=45, validators=[validateLengthGreaterThanTwo], label='Alias:')
    password = forms.CharField(max_length=100, widget=forms.PasswordInput, validators=[validateLengthAtLeastEight, validateMustContainUpper, validateMustContainNumber], label='Password:', help_text="*Password should be at least 8 characters.")
    confirm_password = forms.CharField(max_length=100, widget=forms.PasswordInput, label='Confirm PW')
    birthdate = forms.DateField(label='Date of Birth:', widget=SelectDateWidget(years=range(1930, 2017)))
    class Meta:
        model = User
